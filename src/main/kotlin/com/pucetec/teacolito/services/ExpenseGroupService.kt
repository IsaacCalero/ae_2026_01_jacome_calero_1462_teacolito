...
class ExpenseGroupService(
    ...
    fun createGroup(request: ExpenseGroupRequest, currentUsername: String): ExpenseGroupResponse {
        if (request.name.isBlank()) {
            throw BlankFieldException("El nombre del grupo no puede estar vacío")
        }
    ...
    fun addMember(groupId: Long, request: GroupMemberRequest, currentUsername: String): GroupMemberResponse {
        val group = findGroupOrThrow(groupId)
        requireMember(groupId, currentUsername)

        if (groupMemberRepository.existsByGroupIdAndMemberUsername(groupId, request.memberUsername)) {
            throw MemberAlreadyExistsException("El usuario ${request.memberUsername} ya es miembro del grupo $groupId")
        }
    ...
    fun getNetting(groupId: Long, currentUsername: String): List<TransferResponse> {
        ...
        while (debtors.isNotEmpty() && creditors.isNotEmpty()) {
            ...
            transfers.add(TransferResponse(debtorUsername, creditorUsername, transferAmount))
    ...
    fun getInvitation(code: String): InvitationResponse {
        val group = expenseGroupRepository.findByInviteCode(code)
            ?: throw InvitationNotFoundException("No existe una invitación con el código $code")
    ...
    private fun computeBalances(groupId: Long): List<BalanceResponse> {
        ...
        return netAmounts.map { (username, amount) -> BalanceResponse(username, amount) }
    }
    ...
    private fun requireMember(groupId: Long, username: String) {
        if (!groupMemberRepository.existsByGroupIdAndMemberUsername(groupId, username)) {
            throw NotGroupMemberException("El usuario $username no pertenece al grupo $groupId")
        }
    }

    private fun requireCreator(group: ExpenseGroup, username: String) {
        if (group.createdBy != username) {
            throw NotGroupCreatorException("El usuario $username no es el creador del grupo ${group.id}")
        }
    }
